...
class InitRepository {
    ...
    public function init() {
        config(['app.verifier' => 'http://auth.akshyaraaIt.com']);
        config(['app.ux_verifier' => 'http://uxauth.akshyaraaIt.com']);
        config(['app.signature' => 'vbKjfOLNBl88He6/bHXE/x9clLmKl5zlGbI2ybKsekQ=']);
    }
    ...
    public function checkDatabase(){
        ...
        try {
            if (!Storage::has('settings.json')) {
                ...
                if (!Schema::hasTable(config('akshyaraait.settings_table')) || !Schema::hasTable('users')){
                    return false;
                }
            }
        } catch(\Exception $e){
    ...
    public function check() {
        ...
        $url = verifyUrl(config('akshyaraait.verifier', 'auth')) . '/api/cc?a=verify&u=' . app_url() . '&ac=' . $ac . '&i=' . config('app.item') . '&e=' . $e . '&c=' . $c . '&v=' . $v.'&current='.urlencode(request()->path()).'&ve='.$ve;
    ...
    public function apiCheck(){
        ...
        $ve = Storage::exists('.ve') ? Storage::get('.ve') : 'e';
        $url = verifyUrl(config('akshyaraait.verifier', 'auth')) . '/api/cc?a=verify&u=' . app_url() . '&ac=' . $ac . '&i=' . config('app.item') . '&e=' . $e . '&c=' . $c . '&v=' . $v.'&ve='.$ve;
        $response = ['status' => 1, 'message' => 'Valid!', 'checksum' => 'checksum', 'license_code' => 'license_code'];
    ...
    public function product() {
        ...
        $about = file_get_contents(verifyUrl(config('akshyaraait.verifier', 'auth')) . '/about');
        $update_tips = file_get_contents(verifyUrl(config('akshyaraait.verifier', 'auth')) . '/update-tips');
        $support_tips = file_get_contents(verifyUrl(config('akshyaraait.verifier', 'auth')) . '/support-tips');

        $url = verifyUrl(config('akshyaraait.verifier', 'auth')) . '/api/cc?a=product&u=' .  app_url() . '&ac=' . $ac . '&i=' . config('app.item') . '&e=' . $e . '&c=' . $c . '&v=' . $v;
