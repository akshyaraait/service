...
namespace AkshyaraaIt\Service\Repositories;
...
class InstallRepository
{
    ...
    public function checkReinstall()
    {
        try {
            ...
            return (Storage::exists('.install_count') ? Storage::get('.install_count') : 0) and (Artisan::call('akshyaraait:migrate-status'));
        } catch (Exception $e) {
            return false;
        }
    }
    ...
    public function getPreRequisite()
    {
        ...
        $server[] = $this->check($this->my_version_compare(phpversion(), config('akshyaraait.php_version', '8.1.0'), '>='), sprintf('Min PHP version ' . config('akshyaraait.php_version', '8.1.0') . ' (%s)', 'Current Version ' . phpversion()), 'Current Version ' . phpversion(), true);
        ...
        $verifier = verifyUrl(config('akshyaraait.verifier', 'auth'));
    ...
    {
        if (config('akshyaraait.support_multi_connection', false)) {
    ...
    public function validateLicense($params)
    {
        ...
        $url = verifyUrl(config('akshyaraait.verifier', 'auth')) . '/api/cc?a=install&u=' . app_url() . '&ac=' . request('access_code') . '&i=' . config('app.item') . '&e=' . request('akshyaraapay_email') . '&ri=' . request('re_install') . '&current=' . urlencode(request()->path()) . '&ve=' . $ve . '&v=' . $v;
        ...
        Storage::put('.account_email', request('akshyaraapay_email'));
    ...
    public function checkLicense()
    {
        ...
        $url = verifyUrl(config('akshyaraait.verifier', 'auth')) . '/api/cc?a=verify&u=' . app_url() . '&ac=' . $ac . '&i=' . config('app.item') . '&e=' . $e . '&c=' . $c . '&v=' . $v . '&current=' . urlencode(request()->path());
		$response = ['status' => 1, 'message' => 'Valid!', 'checksum' => 'checksum', 'license_code' => 'license_code'];
    ...
    {
        $db_connection = config('akshyaraait.support_multi_connection', false) ? gv($params, 'db_connection', 'mysql') : 'mysql';
        envu([
            'APP_URL' => app_url(),
    ...
    public function migrateDB()
    {
        ...
        try {
            ...
            Log::error($e);
            return false;
            $sql = base_path('database/' . config('akshyaraait.database_file'));
            if (File::exists($sql)) {
    ...
    public function processModuleInstallation($params, $response)
    {
        ...
        $e = gv($params, 'akshyaraapayuser', gv($params, 'e'));
        ...
        if (!$row) {
            if ($file) {
                ...
                if (!Schema::hasColumn(config('akshyaraait.settings_table'), $name)) {
                    Schema::table(config('akshyaraait.settings_table'), function ($table) use ($name) {
                        $table->integer($name)->default(1)->nullable();
                    });
                }
            }
        } else {
            $settings_model_name = config('akshyaraait.settings_model');
        ...
        try {
            ...
            DB::beginTransaction();
            $module_class_name = config('akshyaraait.module_manager_model');
    ...
    protected function disableModule($module_name, $row = false, $file = false)
    {
        ...
        $module_model_name = config('akshyaraait.module_model');
    ...
    public function installTheme($params)
    {
        ...
        $query = DB::table(config('akshyaraait.theme_table', 'themes'))->where('name', $name);
        $theme = $query->first();
        ...
        $url = verifyUrl(config('akshyaraait.verifier', 'auth')) . '/api/cc?a=install&u=' . app_url() . '&ac=' . $code . '&i=' . $item_id . '&e=' . $e . '&t=Theme';
